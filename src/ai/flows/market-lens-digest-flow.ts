...
// import type { TrendingHeadline } from "@/types"; // Unused
...
const MarketLensDigestInputSchema = z.object({
  headlines: z
    ...
    .describe('An array of trending headlines to process.'),
  region: z
    .string()
    .optional()
    .describe(
      'Optional region to focus the digest on (e.g., India, US, Global). Currently informational.',
    ),
});
...
const DigestedHeadlineSchema = z.object({
  ...
  aiSummary: z
    .string()
    .describe('A concise 1-sentence AI-generated summary of the headline.'),
  source: z.string(),
  publishedAt: z.string(),
  sentiment: z
    .enum(['Bullish', 'Bearish', 'Neutral'])
    .describe('Sentiment derived from the headline.'),
  trendIcon: z
    .enum(['up', 'down'])
    .optional()
    .describe('Visual cue for the trend, if applicable.'),
  originalUrl: z.string(), // Changed from z.string().url()
});

const MarketLensDigestOutputSchema = z.object({
  digestedHeadlines: z
    .array(DigestedHeadlineSchema)
    .describe('An array of processed headlines with summaries and sentiment.'),
  overallMarketSentiment: z
    .string()
    .describe(
      'A brief overall market sentiment based on the provided headlines (e.g., Cautiously Optimistic, Mixed).',
    ),
});
export type MarketLensDigestOutput = z.infer<
  typeof MarketLensDigestOutputSchema
>;

export async function getMarketLensDigest(
  input: MarketLensDigestInput,
): Promise<MarketLensDigestOutput> {
...
const getMarketLensDigestFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const augmentedHeadlines = output.digestedHeadlines.map((digested) => {
      const original = input.headlines.find(
        (h) => h.id === digested.originalId || h.title === digested.title,
      ); // Match by ID or title as fallback
      return {
        ...digested,
