...
export const GenerateMultipleBlogPostsInputSchema = z.object({
  count: z.number().min(1).max(2).describe('The number of blog posts to generate (max 2 due to performance with images).'),
...
export const GenerateMultipleBlogPostsFlowOutputSchema = z.array(GenerateBlogPostOutputSchema);
export type GenerateMultipleBlogPostsFlowOutput = z.infer<typeof GenerateMultipleBlogPostsFlowOutputSchema>;
