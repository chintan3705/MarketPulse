...
export function BlogPostCard({ post, className, orientation = 'vertical' }: BlogPostCardProps) {
  ...
  return (
    <Card className={cn("overflow-hidden hover:shadow-xl transition-shadow duration-300 group", className, {
      ...
      {post.imageUrl && (
        <div className={cn("relative aspect-video shrink-0", {
      ...
      <div className={cn("flex flex-col flex-grow", {"p-0": orientation === 'horizontal'})}>
        <CardHeader className={cn({"p-4 md:p-6": orientation === 'horizontal'})}>
          <Link href={`/category/${post.category.slug}`}>
            <Badge variant="secondary" className="mb-2 inline-block hover:bg-accent hover:text-accent-foreground transition-colors">{post.category.name}</Badge>
          </Link>
        ...
        <CardContent className={cn("flex-grow", {"p-4 md:p-6 pt-0": orientation === 'horizontal'})}>
          <p className="text-sm text-muted-foreground line-clamp-3">{post.summary}</p>
        </CardContent>
        <CardFooter className={cn("flex flex-wrap items-center justify-between text-xs text-muted-foreground gap-2", {"p-4 md:p-6 pt-0": orientation === 'horizontal'})}>
