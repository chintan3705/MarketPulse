...
export async function generateMetadata(
  { params: { slug, locale: localeString } }: CategoryPageProps,
  _parent: ResolvingMetadata,
): Promise<Metadata> {
  const locale = isValidLocale(localeString) ? localeString : defaultLocale;
  const category = categories.find((cat) => cat.slug === slug);
  const canonicalUrl = `${SITE_URL}${locale === defaultLocale ? "" : "/" + locale}/category/${slug}`;
  ...
  locales.forEach((altLocale) => {
    alternatesLang[altLocale] = `${SITE_URL}${altLocale === defaultLocale ? "" : "/" + altLocale}/category/${slug}`;
  });
