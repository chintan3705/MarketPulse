...
export async function generateMetadata(
  { params: { slug, locale: localeString } }: BlogPostPageProps,
  ...
  const alternatesLang: { [key: string]: string } = {};
    ...
    alternatesLang[altLocale] = `${SITE_URL}${altLocale === defaultLocale ? "" : "/" + altLocale}/blog/${slug}`;
  });
          ...
          <div className="mt-6 md:mt-8 pt-4 md:pt-6 border-t">
            ...
            <div className="flex flex-wrap gap-2">
              {post.tags.map((tag) => (
                <Link
                  key={tag}
                  href={`/${locale}/tags/${tag.toLowerCase().replace(/\s+/g, "-")}`}
                >
                  <Badge
                    variant="outline"
