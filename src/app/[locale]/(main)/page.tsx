...
export async function generateMetadata(
  { params: { locale: localeString } }: HomePageProps,
  _parent: ResolvingMetadata,
): Promise<Metadata> {
  const locale = isValidLocale(localeString) ? localeString : defaultLocale;
  const canonicalUrl = `${SITE_URL}${locale === defaultLocale ? "" : "/" + locale}`;

  const alternatesLang: { [key: string]: string } = {};
  locales.forEach((altLocale) => {
    alternatesLang[altLocale] = `${SITE_URL}${altLocale === defaultLocale ? "" : "/" + altLocale}`;
  });
    ...
    <div>
      ...
      <div className="container py-6 md:py-10">
        ...
        <section className="mt-8 md:mt-12">
          <TrendingTagsSection posts={latestBlogPosts} locale={locale} />
        </section>
