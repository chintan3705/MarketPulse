...
export async function generateMetadata(
  ...
  const canonicalUrl = `${SITE_URL}${locale === defaultLocale ? "" : "/" + locale}/contact`;

  const alternatesLang: { [key: string]: string } = {};
  locales.forEach((altLocale) => {
    alternatesLang[altLocale] = `${SITE_URL}${altLocale === defaultLocale ? "" : "/" + altLocale}/contact`;
  });
...
const ContactPage: React.FC<PageProps> = ({ params: { locale } }) => {
  const linkClasses =
